...
- Сохраняет в Obsidian сразу после явной просьбы `сохрани/запиши/зафиксируй`, без обязательных кнопок.
...
## Анализ снов
Если пользователь описывает сон, Кай отвечает содержательно, но не показывает кнопки автоматически.
- `проанализируй сон` — даёт структурированный разбор в Telegram и **не сохраняет файл автоматически**.
- `сохрани сон` или `сохрани сон плюс анализ` — создаёт Markdown-файл в папке `Сны`; внутри будут `## Текст сна`, `## Анализ` и `## Контекст разговора`.
...
## Сохранение без кнопок
Новый основной режим — explicit-save: Кай не показывает кнопки `Сохранить / Не сохранять` после каждого сна, идеи или заметки. Он разговаривает как живой помощник, а сохраняет сразу только после явной просьбы пользователя.

Примеры явных просьб:
- `сохрани это`
- `запиши это`
- `зафиксируй`
- `сохрани сон`
- `сохрани сон плюс анализ`
- `сохрани весь разговор`
- `сохрани в физику`
- `сохрани в АПВ`

После реального создания Markdown-файла Кай отвечает:
```text
Сохранил в Obsidian ✓
Папка: ...
Файл: ...
```
Кай не говорит `сохранил`, пока файл не создан. Старые callback-кнопки остаются в коде, чтобы старые сообщения Telegram не ломались, но новый flow работает без обязательных кнопок.
