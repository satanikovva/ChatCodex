...
def detect_intent(text: str) -> DetectedIntent:
    ...
    if normalized in {"/help", "/start"} or any(x in normalized for x in ("что ты умеешь", "помощь")):
        return DetectedIntent(
            kind="help",
            reply="Я Кай. Могу предложить сохранить задачу, мысль, сон, наблюдение, физическую идею или APV-эксперимент в Notion. Пока сохраняю только после кнопки.",
        )

    if any(x in normalized for x in ("статус", "ты работаешь", "ты жив")):
        return DetectedIntent(kind="status", reply="Я на связи. Пока умею принимать текст, предлагать сохранение в Notion и ждать подтверждения.")

    if any(x in normalized for x in ("удали", "удалить", "сотри", "убери запись", "удали заметку")):
        return DetectedIntent(
            kind="command_delete",
            reply="Я пока не умею удалять записи из Notion автоматически. На следующем шаге добавим удаление последней сохранённой записи. Сейчас удали её вручную в Notion, если она уже сохранена.",
        )
