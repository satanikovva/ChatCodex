...
class ObsidianSaver:
    ...
    def save_draft(self, draft: Draft) -> dict:
        folder = self._folder_for_draft(draft)
    ...
    def _render_markdown(self, draft: Draft, folder: str) -> str:
        reason_block = ""
        if draft.reason:
            reason_block = f"\n## Почему Кай предложил сохранить\n\n{draft.reason}\n"
        escaped_title = draft.title.replace('"', '\\"')
        return (
            "---\n"
            f"type: {draft.entry_kind.value}\n"
